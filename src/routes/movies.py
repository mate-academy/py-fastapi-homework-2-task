...
async def create_movie(movie_data: MovieCreateRequestShema, db: Session = Depends(get_db)):
    ...
    exisiting_check_movie_result = await db.execute(exisiting_check_movie_query) 
    db.add(movie)
    if exisiting_check_movie_result.scalar_one_or_none():
        raise HTTPException(
...
async def update_movie(
    ...
    if "score" in update_dict and (update_dict["score"] < 0 or update_dict["score"] > 100):
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
