...
async def get_movies(
    ...
    return {
        "movies": [
            MovieDetailSchema.model_validate(movie, from_attributes=True)
            for movie in movies
        ],
        "prev_page": (
...
async def create_movie(
    db: AsyncSession = Depends(get_db), movie: MovieCreateSchema = Body(...)
):
    ...
    try:
        ...
        for actor in movie.actors:
            ...
            if not actor_exists:
                new_actor = ActorModel(name=actor)
                db.add(new_actor)
                await db.flush()
        ...
        for language in movie.languages:
            ...
            if not language_exists:
                new_language = LanguageModel(name=language)
                db.add(new_language)
                await db.flush()
...
@router.get(path="/movies/{movie_id}/", response_model=MovieResponseSchema)
async def retrieve_movie(
    movie_id: int = Path(gt=0), db: AsyncSession = Depends(get_db)
):
    query = (
        select(MovieModel)
        .options(
    ...
    return MovieResponseSchema.model_validate(
        movie_exists,
...
async def delete_movie(
    ...
    await db.delete(movie_exists)
    await db.commit()
    return


@router.patch(path="/movies/{movie_id}/")
async def update_movie(
    ...
    for key, value in update.items():
        if value is None:
            continue
        setattr(movie_exists, key, value)

    await db.commit()
    await db.refresh(movie_exists)
