...
async def get_movies(
    ...
    movies = [MovieListItemSchema.model_validate(movie) for movie in db_movies]
...
async def get_movie(movie_id: int, db: Session = Depends(get_db)):
    movie = (await db.execute(
        select(MovieModel)
        .options(selectinload(MovieModel.country), selectinload(MovieModel.genres), selectinload(MovieModel.actors),
                 selectinload(MovieModel.languages))
...
async def update_movie(movie_id: int, update_data: MovieUpdateRequestSchema, db: Session = Depends(get_db)):
    movie = (await db.execute(select(MovieModel).where(MovieModel.id == movie_id))).scalars().first()
    if not movie:
        raise HTTPException(status_code=404, detail="Movie with the given ID was not found.")
