...
from schemas.movies import MovieListSchema, MovieCreateResponse, MovieCreateRequest

from src.schemas.movies import MovieUpdate
...
def add_movie(movie: MovieCreateRequest, db: Session = Depends(get_db)):
    ...
    for name in movie.languages:
        language = db.query(LanguageModel).filter(LanguageModel.name == name).first()
        if language:
...
def edit_movie(movie: MovieUpdate, movie_id: int, db: Session = Depends(get_db)):
    existing_movie = db.query(MovieModel).filter(MovieModel.id == movie_id).first()
